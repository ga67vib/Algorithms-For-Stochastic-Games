...
#CPUs=[0,1,2,3,4]
CPUs=[0] #In this list, put in the CPUs you want to use. We ensure that every command is run on exactly one CPU to avoid things being incomparable due to differences in parallelization
NEXTCPU=0
                    ...
                    for model in sorted(models.keys()):
                        ...
                        for conf in confs:
                            if reps == 1:
                                ...
                                memlogfile = output_dir + "/" + conf + "/" + model + ".time"
                                s1 = "grep 'Time for model checking:' " + logfile
                                s2 = "cut -d ' ' -f 5"
                                ...
                                s1 = "grep 'Maximum resident set size (kbytes)' " + memlogfile
                                s2 = "cut -d ' ' -f 6"
