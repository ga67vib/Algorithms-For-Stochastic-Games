...
public class ProbModelChecker extends NonProbModelChecker
{
	...
	// Int to remember the options for the SMG SolnMethod
	protected int solnMethodOptions = 0;
	...
	public enum SolnMethod {
		VALUE_ITERATION, INTERVAL_ITERATION, POLICY_ITERATION, QUADRATIC_PROGRAMMING, GAUSS_SEIDEL, SOUND_VALUE_ITERATION, OPTIMISTIC_VALUE_ITERATION;
	};
	...
	public ProbModelChecker(PrismComponent parent) throws PrismException
	{
		...
		if (settings != null) {
			...
			// SOLN_METHOD // Added by Maxi 15.05.20. This seems to be the way to make an SMG/STPG-Model checker figure out what it should do. Somehow they haven't implemented it yet, since they always use VI
			s = settings.getString(PrismSettings.PRISM_SMG_SOLN_METHOD);
			if (s.equals("Value iteration")) {
				setSolnMethod(SolnMethod.VALUE_ITERATION);
			} else if (s.equals("Policy iteration")) {
				setSolnMethod(SolnMethod.POLICY_ITERATION);
			} else if (s.equals("Interval iteration")) {
				setSolnMethod(SolnMethod.INTERVAL_ITERATION);
			} else if (s.equals("Quadratic programming")) {
				setSolnMethod(SolnMethod.QUADRATIC_PROGRAMMING);
			} else if (s.equals("Gauss-Seidel")) {
			...
			// SOLN_METHOD_OPTIONS // Added by Maxi 15.05.20. Kinda hacky, since just an int, but nicest way to pass options to all the new methods easily.
			this.solnMethodOptions = settings.getInteger(PrismSettings.PRISM_SMG_SOLN_OPTIONS);
			// PRISM_TERM_CRIT
